...
from src.do_tool.ReasonModeTools.tool_can_use import get_all_tool_definitions, get_tool_instance
...
class ToolUser:
    ...
    async def _build_tool_prompt(
        ...
        prompt += "你现在需要对群里的聊天内容进行回复，现在选择工具来对消息和你的回复进行处理，你是否需要额外的信息，比如联网搜索比较有时效性的信息，或者通过联网搜索来查找你可能不知道的信息"
        return prompt
    ...
    async def _execute_tool_call(self, tool_call, message_txt: str):
        ...
        try:
            ...
            if function_name == "internet_search" :
                ...
                pattern = r"搜|查|search|find"
                if re.search(pattern, message_txt, flags=re.IGNORECASE) is not None:
                    possibility *= 2

                pattern = r"联网搜索|网络搜索|网上查"
                if re.search(pattern, message_txt, flags=re.IGNORECASE) is not None:
                    possibility = 1

                if random.random() > possibility:
                    logger.info(f"因设置原因忽略本次联网搜索,搜索概率{possibility}")
                    return None
